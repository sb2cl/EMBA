...
%   ODE model with Antithetic controller
...
dxdt(m+1,1)= p.pa*p.CNa*p.ka./(p.dma+p.mu)*( p.alpha +...
             ...
             p.k_c./p.kdc*x(m)*x(m+1) +p.k_c*x(m+2)-(p.da+p.mu)*x(m+1);

%x3 = Complex Sigma-Antisigma
dxdt(m+2,1) = p.k_c/p.kdc*x(m)*x(m+1) - p.k_c*x(m+2)-(p.dc+p.mu)*x(m+2);
...
%x6 = CHS
c6 = p.CNh*p.kh./( p.dmh+p.mu );
dxdt(m+5,1) = c6*p.beta*p.phc+ c6*p.ph*( p.alpha + ...
              (1-p.alpha)*x(m)^2./( p.kd20*(p.kds*p.CNh) + x(m)^2 ) )-...
              (p.dh+p.mu)*x(m+5);
